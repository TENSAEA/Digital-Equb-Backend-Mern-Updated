...
const getUnreadNotificationCount = async (req, res) => {
    try {
      ...
      res.status(200).json({ message: 'Unread notifications count retrieved', count: unreadCount });
    } catch (error) {
...
const updateNotification = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Server error while updating notification' }); // General server error
  }
};
  ...
  try {
    ...
    for (const invitation of invitations) {
      ...
      if (!recipient || !message) {
        return res.status(400).json({ message: 'Each invitation must have a recipient and message' });
      }
    ...
    res.status(201).json({ message: 'Notifications sent successfully', notifications: newNotifications });
  } catch (error) {
