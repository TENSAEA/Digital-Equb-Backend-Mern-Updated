...
const createEqub = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'Equb created successfully', equb: newEqub });
  } catch (error) {
...
const getUserCreatedEqubs = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'User-created Equbs fetched successfully', equbs });
  } catch (error) {
...
const joinEqub = async (req, res) => {
  try {
    ...
    if (equb.members.includes(userId)) {
      return res.status(400).json({ message: 'You are already a member of this Equb' });
    }
...
const getUserEqubs = async (req, res) => {
  try {
    ...
    const equbs = await Equb.find({ members: userId })
...
const deleteEqub = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Equb marked for deletion. Awaiting admin confirmation.', equb });
  } catch (error) {
